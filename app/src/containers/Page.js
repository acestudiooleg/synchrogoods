...
const mapActions = dispatch => ({
  triggerLoaded: ({ name }) =>
    dispatch({ type: 'driverapp/PAGE_LOADED', name }),
  onChange: ({ name, wait }) =>
    dispatch({ type: 'driverapp/PAGE_CHANGE', name, wait }),
  onOpen: ({ name, payload }) =>
    dispatch({
      type: `driverapp/PAGE_${name}_OPEN`,
...
export class Internal extends PureComponent {
  ...
  componentDidUpdate(props) {
    if (!isEqual(props.payload, this.props.payload)) {
      if (
        props.payload.url === this.props.payload.url &&
...
export class Page extends Component {
  ...
  render() {
    const props = R.omit(
      ['onChange', 'onOpen', 'validate', 'state', 'name', 'component'],
      this.props,
    );
